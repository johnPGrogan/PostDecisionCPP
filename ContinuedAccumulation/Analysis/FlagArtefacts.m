% function FlagArtefacts()


outFolder = './Saves';
load(fullfile(outFolder, 'ExtractEpochs.mat'));

fileInfo.interpFolder = 'D:\TCD\Projects\ContinuedAccumulation\Data\Interp\'; % my channels
...
art.windowTime = [-400 1500]; % in msec, the limits of the time window relative to the event in which you will check for an artifact
...
figure();
h = errorBarPlot(permute(groupMeans(veog,3,isGood(:,iPP),'dim'),[2,1,3]),'area',1);
...
save(fullfile(outFolder, 'FlagArtefacts3.mat'), ...
