...
% extracted and examines the data for channels that are abnormally high in variance
% 
...
art.windowTime = [-200 2500]; % in msec, the limits of the time window relative to the event in which you will check for an artifact
...
%     SDr = sq(nanstd(conc - nanmean(conc,1),[],2));
    ...
    isArt = sq(max(abs(data.erp(1:eeg.nChans, isBetween(eeg.epochTimes, art.windowTime),:)),[],2) - 100 > 0);  % is uV over 100 in window?

    chanArts = sum(isArt,2); % arts per channel
    chanArtsPerBlock = sq(sum(reshape(isArt,eeg.nChans,fileInfo.nTr,fileInfo.nBlocks),2));
...
%     legend(num2str(blockNums'),'Location','Best');
    ...
    hold on; plot(fileInfo.nTr:fileInfo.nTr:fileInfo.maxTr,eeg.nChans, 'kx')
%     horizontal stripes are bad channels, vertical are bad trials/times
%     
%     figure(); 
%     imagesc(SDct(importantChans,:), sdLims.*[1 1.2]); 
%     hold on; plot(fileInfo.nTr:fileInfo.nTr:fileInfo.maxTr,max(ylim), 'kx')
...
%     plot artefacts too
    figure();
    imagesc(chanArtsPerBlock, [0 fileInfo.nTr]); colorbar
    title('artefacts');
...
%     elec2plot = [2 14 17 23 36 55 56]; figure; hold on; for q=1:length(elec2plot), plot((q-1)*100+conc(elec2plot(q),:)); end; legend(num2str(elec2plot'))
    ...
    % use 
    % [bl,ch] = find(SD(1:eeg.nChans,:)'>sdLims(2)); sortrows([bl,ch])
